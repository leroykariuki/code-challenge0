...
function caclculateDeduction(grossPay){
    let deduction = 0;

    if(groosPay >= 6000 && grossPay <=7999){
        deduction = 300;
...
function caclculatePensionablePayTier(pay){
...
function calculateTaxRates(monthlyTaxablePay){
    let rate = 0;
...
//Calculate of net salary

function calculateNetSalary(salary,benefits){
    var monthlyTaxablePay = salary

    var deductTax = calculateTaxRates(monthlyTaxablePay)//%

    var groosPay = salary - ((deductTax / 100)*salary)

    var deductNhif = caclculateDeduction(groosPay)
